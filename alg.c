...
static int cmp(const char * const s, const size_t i1, const size_t i2) {
	int r = 0;
	char *pi1, * pi2;
	pi1 = &(s[i1]);
	pi2 = &(s[i2]);
...
static void block(const char * const s, int * const a) {
	int n = strlen(s);
	int l = -1;
	int r = -1;
	a[0] = 0;
...
static long long fhash(const char * const s, const long long i1, const long long i2) {
...
static long long find_i(const char * const s, const char * const subs) {
...
static long long is_prime(const long long i) {
...
static long long get_module(const char * const s, const char * const subs) {
...
static long long chash(const char * const s, const long long i1, const long long i2, const long long q) {
	unsigned long long r = (unsigned char)s[i1];
	for (long long i = i1 + 1; i <= i2; ++i) {
		r = (r << 8) % q;
		r = (r + (unsigned char)s[i]);
...
static void prefix_arr(int * const a, const char * const s) {
	a[0] = 0;
	int i = 1;
	int j = 0;
	while (s[i] != SEP) {
...
static void prefix_up(int * const a, const char * const s) {
	int n = strlen(s);
	for (int i = 1; i < n - 1; ++i) {
		if (s[a[i] + 1] == s[i + 1])
			a[i] = a[a[i]];
...
static void ini(int * const a, const size_t n, const int x) {
...
static void bmh_shift_arr(int * const a, const char * const subs) {
	int n = strlen(subs);
	ini(a, ALPHABET, n);
	int i2 = n - 2;
	int i1 = 0;
...
static int is_palindrom(const char *p1, const char *p2) {
	while (p1 <= p2) {
		if (*p1 != *p2)
			return(0);
		++p1;
...
static long long odd_count(const char * const s) {
	char *fp = &(s[strlen(s) - 1]);
	long long r = 0;
...
static long long even_count(const char * const s) {
	char *fp = &(s[strlen(s) - 1]);
	long long r = 0;
	for (char *middle = s; middle <= fp; ++middle) {
		char *left = middle;
...
static void hash_prefix_arr(const char * const s, unsigned long long * const a, const unsigned long long p) {
...
static void hash_suffix_arr(const char * const s, const int n, unsigned long long * const a, const unsigned long long p) {
	unsigned long long h = 0;
	int i = n - 1;
	char *sp = s;
	char* fp = s + n - 1;
...
static void hash_degree_p(unsigned long long * const a, const size_t n, const unsigned long long p) {
...
static int is_pal(const int l, const int r, const int n, unsigned long long * const a, unsigned long long * const b, unsigned long long * const c) {
	int pal_size = r - l + 1;
	int before_pal = l - 1;
	int after_pal = r + 1;
